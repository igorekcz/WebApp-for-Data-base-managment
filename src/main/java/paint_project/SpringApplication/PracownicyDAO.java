...
public class PracownicyDAO {
    ...
    public void save(Pracownicy pracownicy) {
        ...
        insert.withTableName("pracownicy").usingColumns("imie","drugie_imie","nazwisko","data_urodzenia","pesel",
                "plec","adres_email","stanowisko","data_zatrudnienia","nr_konta","numer_telefonu","id_oceanarium","id_sklepu","id_adresu","id_uzytkownika");
    ...
    public Pracownicy getByUserId(int id) {
        Object[] args = {id};
        String sql = "SELECT * FROM PRACOWNICY WHERE id_uzytkownika = " + args[0];
        Pracownicy pracownicy = jdbcTemplate.queryForObject(sql,
                BeanPropertyRowMapper.newInstance(Pracownicy.class));
        return pracownicy;
    }
    ...
    public void update(Pracownicy pracownicy) {
        String sql = "UPDATE PRACOWNICY SET imie=:imie, drugie_imie=:drugie_imie, nazwisko=:nazwisko, data_urodzenia=:data_urodzenia,"+
                ...
                "nr_konta=:nr_konta, numer_telefonu=:numer_telefonu,id_oceanarium=:id_oceanarium,id_sklepu=:id_sklepu,id_adresu=:id_adresu,id_uzytkownika=:id_uzytkownika"+
